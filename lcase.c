...
lcase_sse_simple(char *restrict dst, const char *restrict src, size_t len)
{
	...
	for (const char *cur = src; cur != (src + len); cur += 16, dst += 16)
		STORE_SI128((__m128i *)dst, _mm_or_si128(LOAD_SI128((__m128i *)cur), _mm_set1_epi8(CASE_BIT)));
}
...
lcase_sse_cond(char *restrict dst, const char *restrict src, size_t len)
{
	...
	for (const char *cur = src; cur != (src + len); cur += 16, dst += 16) {
		__m128i data = LOAD_SI128((__m128i *)cur);
		STORE_SI128((__m128i *)dst, _mm_or_si128(data, 
			_mm_and_si128(_mm_and_si128(_mm_cmpgt_epi8(data, _mm_set1_epi8('A' - 1)), 
				_mm_cmpgt_epi8(_mm_set1_epi8('Z' + 1), data)), _mm_set1_epi8(CASE_BIT))));
	}
}
