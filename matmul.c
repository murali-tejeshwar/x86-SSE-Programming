...
{
        int i, j, k;
	__m128 val_a, val_b, val_c, add_res, mul_res;
	...
	for (i = 0; i < SIZE; i++) {
		for (k = 0; k < SIZE; k++) {
			val_a = _mm_load_ps1(&mat_a[i][k]);
			for (j = 0; j < SIZE; j += 4) {
				val_b = _mm_load_ps(&mat_b[k][j]);
				val_c = _mm_load_ps(&mat_c[i][j]);
				mul_res = _mm_mul_ps(val_a, val_b);
				add_res = _mm_add_ps(val_c, mul_res);
				_mm_store_ps(&mat_c[i][j], add_res);
			}
		}
	}
}
